...
function InputGroup({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      role="group"
      className={cn(
        "group/input-group relative flex h-11 w-full min-w-0 items-center md:h-[45px]",
        "rounded-[8px] border-2 border-border bg-input-bg",
        "transition-colors outline-none",
        "hover:border-input-hover",
        "has-[[data-slot=input-group-control]:focus-visible]:border-input-focus",
        "has-[[data-slot][aria-invalid=true]]:border-error has-[[data-slot][aria-invalid=true]]:bg-error-bg",
        "has-[textarea]:h-auto has-[textarea]:rounded-[7px]",
...
const inputGroupAddonVariants = cva(
  "flex h-auto cursor-text items-center justify-center gap-2 py-2 text-sm font-medium text-muted-foreground select-none group-data-[disabled=true]/input-group:opacity-50 **:data-[slot=kbd]:rounded-4xl **:data-[slot=kbd]:bg-muted-foreground/10 **:data-[slot=kbd]:px-1.5 [&>svg:not([class*='size-'])]:size-4",
  {
    variants: {
      align: {
        "inline-start":
        "order-first pl-3 has-[>button]:-ml-1 has-[>kbd]:ml-[-0.15rem] [&>svg]:size-3.5",
        "inline-end":
  ...
  return (
    <Input
      ...
      className={cn(
        ...
        "aria-invalid:border-0 aria-invalid:bg-transparent",
        className
      )}
      {...props}
    />
